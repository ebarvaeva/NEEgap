# =============================================================================
# RF_Grazing_CV.R — Random Forest: BASE + Grazing_days_since  (PI disabled)
# =============================================================================
#
# PURPOSE
#   Variant of RF_CV.R for the management variable ablation study.
#   Identical in all respects to RF_CV.R except that:
#
#     PI_ENABLED <- FALSE   # Hardcoded: Grazing_days_since is a direct predictor here, not a PI trigger
#
#   is hardcoded (line ~165), regardless of whether Grazing_days_since
#   is present in the predictor set.  This allows Grazing_days_since to
#   be passed as a direct numeric split predictor without triggering the
#   per-gap Phytomass Index computation.
#
#   This script is called by run_management_effect_GrazingDaysSince.R,
#   which supplies:
#     predictors = BASE + Grazing_days_since
#     PI_ENABLED = FALSE  (also injected via run_env, consistent with
#                          the hardcoded value here)
#
# RATIONALE
#   Keeping PI_ENABLED = FALSE here (rather than relying solely on the
#   run_env injection) makes the script self-contained: sourcing it
#   directly in any context will never accidentally activate the
#   Phytomass Index, preventing unintended mixing of predictor variants.
#
