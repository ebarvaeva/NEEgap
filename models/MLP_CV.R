# =============================================================================
# MLP_CV.R — Multi-Layer Perceptron Cross-Validation Gap-Filling
#             NEE gap-fill with post-hoc Reco and GPP derivation
# =============================================================================
#
# PURPOSE
#   Trains a fully-connected feed-forward neural network (Multi-Layer
#   Perceptron, implemented via Keras / TensorFlow) separately for each
#   artificial gap in each of four gap-size categories (S, M, L, VL).
#   The network is trained on observations outside each gap and used to
#   predict NEE at the gap rows.  Reco and GPP are derived post-hoc from
#   the gap-filled NEE series through flux partitioning (see Sections 4-6
#   of RF_CV.R, which are duplicated here for self-containment).
#
# NETWORK ARCHITECTURE  (defined in build_mlp(), Section 7)
#   Input layer:  n_features neurons
#   Hidden layer 1: 128 neurons, ReLU activation
#   Hidden layer 2:  64 neurons, ReLU activation
#   Hidden layer 3:  32 neurons, ReLU activation
#   Output layer:     1 neuron, linear (regression)
#   Loss: mean squared error (MSE); monitored metric: mean absolute error
#   Optimiser: Adam with default learning rate
#   Early stopping: patience = 10 epochs on validation loss,
#                   restore best weights
#
# FEATURE AND TARGET SCALING  (Section 7)
#   All predictors are min-max scaled to [0, 1] using training-set
#   statistics, with out-of-range values clipped.  Missing values in
#   predictors are replaced by the training-set median.  The target NEE
#   is also min-max scaled so that the output layer operates in [0, 1];
#   predictions are unscaled before being written to the output columns.
#   Scaling is essential for gradient-based optimisers to converge
#   efficiently; it is a standard requirement for neural networks unlike
#   for tree-based models.
#
# TRAIN / VALIDATION SPLIT  (stratified_train_val_split(), Section 7)
#   10% of training rows are held out as a validation set for early
#   stopping.  The split is stratified by season × month × day/night to
#   ensure the validation set covers the full range of conditions present
#   in the training data.  This prevents early stopping from being driven
#   by an unrepresentative validation sample.
#
...
#   rds_name    — source file name
#
# OUTPUTS  (written to RESULTS_DIR)
#   df_cv_all_predictions.rds
#     Original data frame with 12 new columns:
#       NEE_{S|M|L|VL}_mlp_predicted
#       Reco_{S|M|L|VL}_mlp_predicted
#       GPP_{S|M|L|VL}_mlp_predicted
#
#   training_loss_curves/
#     MLP_NEE_{SIZE}_{LABEL}_{loss|mae}.png  — training and validation
#     loss curves for each gap label (MSE and MAE in scaled units)
#
#   progress.log | run_info.txt
#
# SECTIONS
#    1  Reproducibility           — random seeds for R and Python/TensorFlow
...
#    7  MLP-specific helpers      — scaling, train/val split, architecture
#    8  MLP cross-validation      — run_mlp_for_gap_size() definition
...
#   12  Keras / TensorFlow init   — load Python environment and fix seeds
#   13  Run MLP cross-validation  — loop over gap sizes
#   14  Derive Reco and GPP       — post-hoc partitioning
#   15  Save outputs
#
# =============================================================================
...
# =============================================================================
# Neural network training involves stochastic operations in both R
# (weight initialisation via the base RNG) and Python / TensorFlow
# (PYTHONHASHSEED, numpy random, tf.random).  All seeds are fixed to 42
# and CPU-only execution is forced (CUDA_VISIBLE_DEVICES = "-1") to
# eliminate GPU non-determinism.  Thread counts are fixed at 1 for the
# same reason as in RF_CV.R.  The Python-side seeds are set again in
# Section 12 after the reticulate environment is initialised.
...
# =============================================================================
# Identical in purpose to Section 3 of RF_CV.R.  Writes timestamped
# messages to the console and to progress.log / progress_log.rds.
...
# =============================================================================
# Gap construction, metric computation, and season labelling functions.
# These are identical in purpose to Section 4 of RF_CV.R; refer to that
# script for detailed documentation of each function.
...
# =============================================================================
# Identical in purpose to Section 5 of RF_CV.R.  The add_phytomass_index()
# function computes one PI_{gap_label} column per gap label, quantifying
# sward regrowth state from outside-gap night and day NEE observations.
# For full algorithm documentation see RF_CV.R Section 5.
...
# =============================================================================
# Assigns an integer regrowth_id to each row based on grazing events.
# For full documentation see RF_CV.R Section 6.
...
# =============================================================================
# Three functions shared by ground-truth computation and gap-row prediction:
#   compute_reco_gpp_from_nee_orig() — partition observed NEE into
#     Reco_orig and GPP_orig per regrowth period (called once, ground truth)
#   derive_reco_gpp_from_filled_nee() — for each gap label, reconstruct a
#     complete NEE series, partition it, and write gap-row values to the
#     Reco/GPP prediction columns.
# For full algorithm documentation see RF_CV.R Section 7.
#
...
# =============================================================================
# SECTION 7 — MLP-specific helpers
# =============================================================================
# Six functions that handle the MLP-specific aspects of the pipeline:
#
# fit_feature_scaler(df, train_rows, feature_cols)
#   Computes min, max, and median of each feature over the training rows.
#   Missing values are imputed with the training-set median; the range
#   (max - min) is used for scaling.  Returns a scaler list.
#
# apply_feature_scaler(df, row_idx, scaler)
#   Applies the scaler: (x - min) / range, clipped to [0, 1].
#   Missing values in the transform set are replaced by the training median
#   before scaling.  Returns a numeric matrix ready for Keras.
#
# fit_target_scaler(y) / scale_target(y, s) / unscale_target(ys, s)
#   Min-max scaling for the NEE target.  Fitted on training rows only;
#   applied to training and validation targets, then inverted for output.
#
# stratified_train_val_split(flux_data, obs_rows, val_frac, seed)
#   Holds out val_frac (default 10%) of training rows as a validation set.
#   Stratification is performed jointly by season, month, and day/night
#   flag so that the validation set has proportional representation of all
#   condition combinations.  Half of the validation budget goes to daytime
#   rows and half to night-time rows.
#
# build_mlp(n_features, hidden)
#   Constructs a Keras sequential model: three ReLU hidden layers of sizes
#   128, 64, 32 followed by a linear output neuron.  Compiled with the
#   Adam optimiser and MSE loss.  hidden can be modified to change the
#   architecture.   (default: c(128L, 64L, 32L))
#
# save_mlp_loss_curves(history_df, gap_size_cat, gap_label)
#   Saves training and validation MSE and MAE curves as PNG files in
#   training_loss_curves/.  Loss is reported in scaled [0,1] units.
...
# =============================================================================
# run_mlp_for_gap_size() iterates over every gap label within one gap-size
# category.  For each label:
#   1. Identify training rows (finite NEE outside the gap).
#   2. Append PI_{gap_label} to the feature set if PI is enabled.
#   3. Split training rows into train / validation with stratified sampling.
#   4. Fit feature and target scalers on the training partition.
#   5. Scale features and targets for train, validation, and gap rows.
#   6. Build and fit the MLP with early stopping on validation loss.
#   7. Save training loss curves.
#   8. Predict and unscale NEE at gap rows.
# Predictions are written to the NEE_{SIZE}_mlp_predicted column.
...
# =============================================================================
# Filter to rows with finite NEE_orig (the training universe).  Check
# whether PPFD and Temp are available for Reco/GPP partitioning.
# Assign regrowth periods from Grazing_days_since if present, otherwise
# use a single global period.  Compute ground-truth Reco_orig / GPP_orig.
...
# For gap-size specifications see RF_CV.R Section 10.
...
# =============================================================================
# Compute per-gap PI columns if enabled.  Must be called after all gap
# matrices are constructed so that build_pi() can identify gap rows by
# the flag columns.  See RF_CV.R Section 11 for the window_days parameter.
...
# =============================================================================
# TensorFlow is loaded here (not at the top of the script) because the
# reticulate Python environment must be initialised after R packages are
# loaded.  The virtual environment "r-tensorflow" must exist and contain
# TensorFlow >= 2.x.  GPU execution is explicitly disabled
# (CUDA_VISIBLE_DEVICES = "-1") to ensure determinism on CPU.  All
# Python-side random seeds (numpy, tensorflow, random) are reset here
# to complement the R seed set in Section 1.
# To change the virtual environment name, edit the use_virtualenv() call.
...
# =============================================================================
# Apply run_mlp_for_gap_size() to all four gap-size categories in
# decreasing size order (VL -> L -> M -> S).
...
# =============================================================================
# Post-hoc partitioning of the MLP gap-filled NEE into Reco and GPP.
# Identical procedure to RF_CV.R Section 13.
...
# =============================================================================
# Save the augmented data frame and run metadata.  See RF_CV.R Section 14.
