# =============================================================================
# RF_PI_CV.R — Random Forest: BASE + Phytomass Index  (Grazing not in predictors)
# =============================================================================
#
# PURPOSE
#   Variant of RF_CV.R for the Phytomass Index ablation study.
#   Identical in all respects to RF_CV.R except that:
#
#     PI_ENABLED <- TRUE   # Hardcoded: PI is the sole management predictor in this ablation run
#
#   is hardcoded (line ~165), activating the per-gap Phytomass Index
#   computation even though Grazing_days_since is NOT in the predictor set.
#
#   This script is called by run_management_effect_PI.R, which supplies:
#     predictors = BASE only  (no Grazing_days_since)
#     PI_ENABLED = TRUE       (also injected via run_env for consistency)
#
# RATIONALE
#   By keeping PI_ENABLED = TRUE here independently of the predictor list,
#   the Phytomass Index is computed per-gap even though Grazing_days_since
#   does not appear as a split variable.  This isolates the PI contribution
#   from the direct effect of Grazing_days_since as a numeric predictor.
#
#   The Phytomass Index requires Grazing_days_since to exist IN THE DATA
#   (to identify regrowth periods), but it must not be passed in predictors.
#   add_phytomass_index() reads Grazing_days_since directly from flux_data;
#   it is not taken from the predictors vector.
#
# For full documentation of all sections, see RF_CV.R.
#
# OUTPUTS  (written to RESULTS_DIR)
#   df_cv_all_predictions.rds, variable_importance/, progress.log, run_info.txt
#
# =============================================================================
...
# =============================================================================
# Active only when "Grazing_days_since" is present in `predictors`.
# Full documentation in the presentation / previous model versions.
...
# =============================================================================
# PURPOSE
#   For each artificial gap label, train RF on outside-gap rows and predict
#   NEE inside the gap.  Variable importance is saved per gap label and
#   aggregated per gap-size category.
#
# IN  : flux_data    — data frame with gap flag and masked NEE columns
#       gap_size_cat — "S" | "M" | "L" | "VL"
#       feature_cols — predictor column names
# OUT : flux_data with column  NEE_{gap_size_cat}_rf_predicted  added
