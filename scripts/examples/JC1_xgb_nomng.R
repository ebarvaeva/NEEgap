# JC1_xgb_nomng.R — Run script for XGBoost NEE gap-filling
#
...
# models/XGBoost_CV.R to gap-fill NEE by cross-validation across four gap-size
# classes (VL = very large, L = large, M = medium, S = small).
#
# Required input columns (in DATA_FILE):
#   timestamp   — POSIXct / parseable half-hourly date-time (with night, stratifies the CV split)
...
#     XGB_NEE_{SIZE}_{LABEL}_rmse.png
#
...
MODEL_CHOICE         <- "XGBoost"    # names the model script (XGBoost_CV.R) and results subfolder
...
suppressPackageStartupMessages({
  library(here); library(dplyr); library(tibble); library(tidyr); library(ggplot2); library(lubridate); library(xgboost)
})
...
# pass settings to XGBoost_CV.R via its own environment, then source it
