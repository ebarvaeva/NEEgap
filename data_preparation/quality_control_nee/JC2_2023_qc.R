# JC2_2023_qc.R — quality-control the half-hourly CO2 flux for JC2 2023
#
# Reads the four EddyPro exports (eddypro, biomet, meta, fluxnet), aligns them on a
# half-hourly Europe/Dublin timestamp and merges. Air temperature and Rg are
# overwritten with gap-filled Met Éireann hourly records. Applies the base
...
#
# Input  : data/raw_data/JC2_2023_{eddypro,biomet,meta,fluxnet}.xlsx
...
# Output : data/data_qc/results_qc_JC2_2023/merged_qc.csv
#          data/data_qc/results_qc_JC2_2023/merged_qc.rds
#          data/data_qc/results_qc_JC2_2023/ustar_seasonal_thresholds.rds
#----------------------------------------------------------
# Packages
library(here)
library(readr)
...
eddypro_path <- here::here("data/raw_data/JC2_2023_eddypro.xlsx")
meta_path    <- here::here("data/raw_data/JC2_2023_meta.xlsx")
fluxnet_path <- here::here("data/raw_data/JC2_2023_fluxnet.xlsx")
biomet_path  <- here::here("data/raw_data/JC2_2023_biomet.xlsx")
...
met_temperature <- met_temperature %>%
  mutate(
    timestamp = dmy_hm(date, tz = "Europe/Dublin")
  ) %>%
...
hh_met <- data.frame(
  timestamp = seq(
    ...
    to   = as.POSIXct("2023-12-31 23:30:00", tz = "Europe/Dublin"),
    by   = "30 min"
  )
) 
...
globrad <- read_excel(here::here("data/meteireann_data/solar_hourly.xlsx")) %>%
  mutate(
    timestamp = ymd_hms(date, tz = "Europe/Dublin")
  ) %>%
  filter(year == as.integer(year_no)) %>%
  mutate(glorad = if_else(is.na(glorad), 0, glorad)) %>%
...
site_key <- "Johnstown Castle - Teagasc (2)"
