# JC1_2024_qc.R — quality-control the half-hourly CO2 flux for JC1 2024
#
# Reads the four EddyPro exports (eddypro, biomet, meta, fluxnet). Timestamps are
# parsed in Europe/Dublin then converted to UTC before merging. Air temperature and
# Rg are overwritten with gap-filled Met Éireann hourly records. Applies the base
# range/instrument filters, a seasonal u* threshold (REddyProc), the 1-9 stationarity
# flag levels, then the 2-D footprint filter (Kljun 2015) against the site boundary
# polygon at 70/80/90 % thresholds. Data preparation reads co2_flux_base_filters_6_70_grid.
#
# Input  : data/raw_data/JC1_2024_{eddypro,biomet,meta,fluxnet}.xlsx
#          data/meteireann_data/{met_hourly.csv,solar_hourly.xlsx}
#          data_preparation/quality_control_nee/grid_qc/nasco_site_list_wkt.csv
# Output : data/data_qc/results_qc_JC1_2024/merged_qc.csv
#          data/data_qc/results_qc_JC1_2024/merged_qc.rds
#          data/data_qc/results_qc_JC1_2024/ustar_seasonal_thresholds.rds
#----------------------------------------------------------
# Packages
library(here)
library(readr)
...
eddypro_path <- here::here("data/raw_data/JC1_2024_eddypro.xlsx")
meta_path    <- here::here("data/raw_data/JC1_2024_meta.xlsx")
fluxnet_path <- here::here("data/raw_data/JC1_2024_fluxnet.xlsx")
biomet_path  <- here::here("data/raw_data/JC1_2024_biomet.xlsx")

site     <- "JC1"
year_no  <- "2024"

# make sure the folder exists
results_path <- here::here("data/data_qc", paste0("results_qc_", site, "_", year_no))
...
eddypro  <- eddypro  %>% mutate(timestamp = with_tz(timestamp, "UTC"))
biomet   <- biomet   %>% mutate(timestamp = with_tz(timestamp, "UTC"))
meta     <- meta     %>% mutate(timestamp = with_tz(timestamp, "UTC"))
fluxnet  <- fluxnet  %>% mutate(timestamp = with_tz(timestamp, "UTC"))
...
#----------------------------------------------------------
# Change air_temperature to Met Eireann's records
met_temperature <- read_csv(here::here("data/meteireann_data/met_hourly.csv")) %>%
  mutate(timestamp = with_tz(dmy_hm(date, tz = "Europe/Dublin"), "UTC")) %>%
  filter(year(timestamp) == as.integer(year_no)) %>%
...
hh_met <- data.frame(
  timestamp = seq(
    ...
    to   = as.POSIXct("2024-12-31 23:30:00", tz = "UTC"),
    by   = "30 min"
  )
)

met_temperature <- hh_met %>%
  left_join(met_temperature, by = "timestamp") %>%
  mutate(air_temperature = na.approx(air_temperature, x = timestamp, na.rm = FALSE, rule = 2))
...
#----------------------------------------------------------
# Rg Met Eireann's records
globrad <- read_excel(here::here("data/meteireann_data/solar_hourly.xlsx")) %>%
  mutate(
    timestamp = with_tz(ymd_hms(date, tz = "Europe/Dublin"), "UTC"),
    glorad    = if_else(is.na(glorad), 0, as.numeric(glorad))
  ) %>%
  filter(year(timestamp) == as.integer(year_no)) %>%
  dplyr::select(timestamp, glorad)

globrad <- hh_met %>%
  left_join(globrad, by = "timestamp") %>%
  mutate(Rg = na.approx(glorad, x = timestamp, na.rm = FALSE, rule = 2)) %>%
  dplyr::select(-glorad)
...
merged <- eddypro %>%
  ...
  rename(u_star = `u*`) %>%
  mutate(air_temperature  = ifelse(air_temperature >= -30 + 273.15 & air_temperature <= 40 + 273.15, air_temperature, NA_real_))
...
ep_df <- merged %>%
  transmute(
    ...
    Tair  = ifelse(is.finite(air_temperature),        air_temperature - 273.15,       NA_real_),
    Rg    = ifelse(is.finite(Rg), Rg, NA_real_)
  )

hh_skeleton <- data.frame(
  DateTime = seq(
    from = as.POSIXct(paste0(year(min(merged$timestamp)), "-01-01 00:30:00"), tz = "UTC"),
    to   = as.POSIXct(paste0(year(min(merged$timestamp)), "-12-31 23:30:00"), tz = "UTC"),
    by   = "30 min"
  )
)
...
EProc <- sEddyProc$new('site', ep_df, c('NEE', 'Ustar', 'Tair', 'Rg'))
...
saveRDS(ustar_th, file = file.path(results_path, "ustar_seasonal_thresholds.rds"))
...
# change this to your site label from the CSV
