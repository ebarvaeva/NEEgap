# quality_control_nee.R — quality-control the half-hourly CO2 flux for one site x year
#
# Reads the four EddyPro exports (eddypro, biomet, meta, fluxnet), aligns them on a
# common half-hourly timestamp and merges into one table. Applies the base
# range/instrument filters, a seasonal u* threshold (REddyProc), the 1-9 stationarity
# flag levels, then a 2-D footprint filter (Kljun 2015): the flux footprint is built
# per half-hour and only half-hours whose footprint mass falls inside the site
# boundary polygon are kept, at 70/80/90 % thresholds. Output holds one masked
# co2_flux column per flag level x footprint threshold; data preparation reads
# co2_flux_base_filters_6_70_grid.
#
# Input  : data/raw_data/{SITE}_{YEAR}_{eddypro,biomet,meta,fluxnet}.xlsx
#          data_preparation/quality_control_nee/grid_qc/nasco_site_list_wkt.csv
# Output : data/data_qc/results_qc_{SITE}_{YEAR}/merged_qc.csv
#          data/data_qc/results_qc_{SITE}_{YEAR}/merged_qc.rds
#          data/data_qc/results_qc_{SITE}_{YEAR}/ustar_seasonal_thresholds.rds
#----------------------------------------------------------
# Packages
library(here)
library(readr)
...
# change the polygon boundary's name for the grid footprint filter
