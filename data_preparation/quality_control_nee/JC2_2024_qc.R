# JC2_2024_qc.R — quality-control the half-hourly CO2 flux for JC2 2024
#
# Reads the four EddyPro exports (eddypro, biomet, meta, fluxnet), aligns them on a
# half-hourly Europe/Dublin timestamp and merges. Air temperature is overwritten with
# gap-filled Met Éireann hourly records; biomet PPFD is interpolated and Rg for the
# u* step is derived from it. Applies the base range/instrument filters, a seasonal u*
# threshold (REddyProc), the 1-9 stationarity flag levels, then the 2-D footprint
# filter (Kljun 2015) against the site boundary polygon at 70/80/90 % thresholds.
#
# Input  : data/raw_data/JC2_2024_{eddypro,biomet,meta,fluxnet}.xlsx
#          data/meteireann_data/met_hourly.csv
#          data_preparation/quality_control_nee/grid_qc/nasco_site_list_wkt.csv
# Output : data/data_qc/results_qc_JC2_2024/merged_qc.csv
#          data/data_qc/results_qc_JC2_2024/merged_qc.rds
#          data/data_qc/results_qc_JC2_2024/ustar_seasonal_thresholds.rds
#----------------------------------------------------------
# Packages
library(here)
library(readr)
...
eddypro_path <- here::here("data/raw_data/JC2_2024_eddypro.xlsx")
meta_path    <- here::here("data/raw_data/JC2_2024_meta.xlsx")
fluxnet_path <- here::here("data/raw_data/JC2_2024_fluxnet.xlsx")
biomet_path  <- here::here("data/raw_data/JC2_2024_biomet.xlsx")
...
hh_met <- data.frame(
  timestamp = seq(
    from = as.POSIXct("2024-01-01 00:00:00", tz = "Europe/Dublin"),
    to   = as.POSIXct("2024-12-31 23:30:00", tz = "Europe/Dublin"),
    by   = "30 min"
  )
) 
...
merged <- eddypro %>%
  ...
  mutate(PPFD_1_1_1_biomet = na.approx(PPFD_1_1_1_biomet, x = timestamp, na.rm = FALSE, rule = 2))
...
ep_df <- merged %>%
  transmute(
    ...
    Rg    = ifelse(is.finite(PPFD_1_1_1_biomet),      PPFD_1_1_1_biomet / 2.0565, NA_real_)
  )
