# run_management_effect_RF.R — Random Forest feature-addition batch runner
# Trains the Random Forest once per management variable, each time on the BASE
# predictor set plus ONE management variable, to isolate that variable's
# marginal effect on NEE gap-filling. PI_ENABLED is always FALSE here (raw
# predictor effect only; use run_management_effect_PI.R for the Phytomass
# Index). RF omits the season dummies used by MLP/XGBoost — seasonality is
# already captured by the sin/cos encodings, and hard 0/1 splits hurt trees.
# Loops over every site in SITES and every variable in MGMT_VARS, sourcing
# models/RF_CV.R in an isolated environment for each combination.
...
# BASE predictor set — no management variables (RF omits season dummies)
BASE_PREDICTORS <- c(
  "PPFD", "Rg",
